...
class Password_Free_Endpoint_User_Auth
{
    ...
    public function password_free_delete_users_password_free_marker($request)
    {
        ...
        global $wpdb;
       $query = $wpdb->prepare(
          "DELETE FROM $wpdb->usermeta WHERE user_id IN (%s) AND meta_key = %s",
             $ids,
    ...
    public function password_free_endpoint_get_password_free_users_func($request)
    {
        ...
        if ($total_count > 0) {
            ...
            $query_users = $wpdb->prepare(
               "select us.ID as userId, us.user_email as email from $wpdb->users us left join $wpdb->usermeta nu on us.ID = nu.user_id where nu.meta_key = %s order by us.ID limit %d, %d",
                  PASSWORD_FREE_CUSTOMER_ID,
                  $offset,
                  $page_size
               );
    ...
    public function password_free_register_user($request)
    {
        ...
        $redirect_code = $this->password_free_generate_string();
    ...
    private function registration_actions($email, $password_free_customer_id) {
        $username = $email;
        $password = $this->password_free_generate_string();
    ...
    public function password_free_redirect_user($request)
    {
        ...
        if (($redirect_code == '' && $redirect_code != $request->get_param('code'))
            ...
            if (is_multisite()) {
                ...
                $url = $this->password_free_get_redirect_url($operation, true, $blog_id, $user_id);
                restore_current_blog();
            } else {
                $url = $this->password_free_get_redirect_url($operation, false, $blog_id, $user_id);
            }
    ...
    private function password_free_get_redirect_url($operation, $is_multisite, $blog_id, $user_id)
    {
    ...
    private function password_free_generate_string(): string
    {
        $code_length = 14;
