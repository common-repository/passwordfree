...
class Password_Free_Endpoint_Inner
{
    ...
    public function password_free_customization_apply_default_func()
    {
        ...
        file_put_contents(PASSWORD_FREE_UPLOAD_DIR_PATH . PASSWORD_FREE_BUTTONS_STYLE_DIR_NAME  .  '/password-free-button-style' . $blog_id . '.css', $data, LOCK_EX);
    }
    ...
    public function password_free_customization_apply_func($request)
    {
        ...
        $this->password_free_update_redirect_url($custom_array['register_url'], $custom_array['auth_url']);
        $data = Password_Free_Buttons_Style::password_free_get_css_data($custom_array, $width, $height);
        $blog_id = is_multisite() ? get_current_blog_id() : '';
        $upload_dir = wp_upload_dir();
        file_put_contents($upload_dir['basedir'] . PASSWORD_FREE_BUTTONS_STYLE_DIR_NAME  .  '/password-free-button-style' . $blog_id . '.css', $data, LOCK_EX);
    }
    ...
    private function password_free_update_redirect_url($register_url, $auth_url) {
        if ($register_url == '') {
            delete_option(PASSWORD_FREE_REDIRECT_AFTER_REGISTER_URL);
        } else {
