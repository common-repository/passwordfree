        ...
        if (function_exists('is_multisite') && is_multisite()) {
            ...
            if ($networkwide) {
                ...
                if ((empty($blog_ids_with_active_plugin))) {
                    $db_params = Password_Free_Activator::password_free_activation_requests();
                } else {
            ...
            password_free_add_db_values_to_blog(Password_Free_Activator::password_free_activation_requests());
            password_free_activation_actions(true);
        }
...
function password_free_deactivate($networkwide)
{
    ...
    if (function_exists('is_multisite') && is_multisite()) {
        ...
        if ($networkwide) {
            ...
            if (empty($blog_ids_with_active_plugin)) {
                Password_Free_Deactivator::password_free_deactivate_request();
            }
            foreach ($blogs as $blog) {
                $blog_id = $blog->blog_id;
                if (!in_array($blog_id, $blog_ids_with_active_plugin)) {
                    switch_to_blog($blog->blog_id);
                    Password_Free_Deactivator::password_free_deactivate();
                    restore_current_blog();
                }
            }
        } else {
                ...
                if (get_option(PASSWORD_FREE_IS_MAIN_URL)) {
                    for ($i = 0; $i <= count($blog_ids_with_active_plugin); $i++) {
                        if ($blog_ids_with_active_plugin[$i] != get_current_blog_id()) {
                            ...
                            $notification->password_free_send_change_url_request($site_url);
...
function password_free_uninstall()
{
    ...
    if (function_exists('is_multisite') && is_multisite()) {
        ...
        Password_Free_Uninstall::password_free_uninstall_request();
        foreach ($blogs as $blog) {
            switch_to_blog($blog->blog_id);
            Password_Free_Uninstall::password_free_uninstall();
            restore_current_blog();
        }
    } else {
        Password_Free_Uninstall::password_free_uninstall_request();
    ...
    $directory = PASSWORD_FREE_UPLOAD_DIR_PATH . PASSWORD_FREE_BUTTONS_STYLE_DIR_NAME;
    Password_Free_Uninstall::password_free_delete_dir_in_uploads($directory);
}
...
function password_free_remote_request(string $url, array $args) {
   return wp_remote_request($url, $args);
}

function password_free_remote_post(string $url, array $args) {
   return wp_remote_post($url, $args);
}

function password_free_remote_get(string $url, array $args) {
