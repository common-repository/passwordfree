...
class Password_Free_Buttons
{
    ...
    {
       echo wp_kses(
          self::password_free_button(true, false, true),
          [
             'div' => [
                'class' => [],
                'onclick' => []
    ...
    public static function password_free_button($isRegister, $is_need_additional_info, $is_need_redirect): string
    {
        if ($is_need_redirect) {
            ...
            if (!file_exists(PASSWORD_FREE_UPLOAD_DIR_PATH . PASSWORD_FREE_BUTTONS_STYLE_DIR_NAME  .  '/password-free-button-style' . $blog_id . '.css')) {
                wp_mkdir_p( PASSWORD_FREE_UPLOAD_DIR_PATH . PASSWORD_FREE_BUTTONS_STYLE_DIR_NAME );
            ...
            wp_enqueue_style("password_free_button-style", PASSWORD_FREE_UPLOAD_URL_PATH . PASSWORD_FREE_BUTTONS_STYLE_DIR_NAME  .  '/password-free-button-style' . $blog_id . '.css');
            wp_enqueue_script("password_free_button-js", PASSWORD_FREE_URL . 'admin/js/password-free-button.js', '', '', true);

            $array = array(
                'pfRegisterRedirectUrl' => self::password_free_get_redirect_url(true),
                'pfAuthRedirectUrl' => self::password_free_get_redirect_url(false),
            );
            wp_localize_script('password_free_button-js', 'buttonVariables', $array);
        }
        $motivation_text = $is_need_redirect ? self::password_free_get_motivation_text($isRegister) : '';
        $text = self::password_free_get_button_text($isRegister);
        return self::password_free_get_buttons_template($text, $is_need_additional_info, $motivation_text, $isRegister);
    }

    private static function password_free_get_buttons_template($text, $is_need_additional_info, $motivation_text, $isRegister): string
    {
    ...
    private static function password_free_get_button_text($isRegister): string
    {
        return $isRegister ? 'Sign up PasswordFree' : 'Sign in PasswordFree';
    }

    private static function password_free_get_redirect_url($isRegister): string
    {
    ...
    private static function password_free_get_motivation_text($isRegister)
    {
